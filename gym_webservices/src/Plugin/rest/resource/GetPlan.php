 ...
 *
 * @RestResource(
 *   id = "get_rest_work_out_plan",
 *   label = @Translation("Workout Plan for Trainee rest123 resource"),
 *   uri_paths = {
...
class GetPlan extends ResourceBase {
   /**
    ...
    if ($flag) {
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'cardio') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle())) {
		$cardiodata[] = [$field_name => $field_definition->getLabel(), 'ActivityDuration' => $para_cardio->$field_name->value];
      }
     }
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'chest') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$chestdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_chest->$field_name->value, 'ActivityReps' => $para_chest->{$field_name."_reps"}->value];
      }
     }
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'back') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$backdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_back->$field_name->value, 'ActivityReps' => $para_back->{$field_name."_reps"}->value];
      }
     } 
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'triceps') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$tricepsdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_triceps->$field_name->value, 'ActivityReps' => $para_triceps->{$field_name."_reps"}->value];
      }
	 }
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'fore_arms') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$forearmsdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_forearms->$field_name->value, 'ActivityReps' => $para_forearms->{$field_name."_reps"}->value];
      }
     } 
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'abs') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$absdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_abs->$field_name->value, 'ActivityReps' => $para_abs->{$field_name."_reps"}->value];
      }
     } 
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'side') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$sidedata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_side->$field_name->value, 'ActivityReps' => $para_side->{$field_name."_reps"}->value];
      }
     }
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'thigh') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$thighdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_thigh->$field_name->value, 'ActivityReps' => $para_thigh->{$field_name."_reps"}->value];
      }
	 }
	  //For Biceps
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'biceps') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$bicepsdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_biceps->$field_name->value, 'ActivityReps' => $para_biceps->{$field_name."_reps"}->value];
      }
     }
	 ...
	 foreach ($this->entityManager->getFieldDefinitions($entity_type_id, 'shoulder') as $field_name => $field_definition) {
      if (!empty($field_definition->getTargetBundle()) && !strpos($field_name,"reps")) {
		$shoulderdata[] = [$field_name => $field_definition->getLabel(), 'ActivitySets' => $para_shoulder->$field_name->value, 'ActivityReps' => $para_shoulder->{$field_name."_reps"}->value];
      }
     }
